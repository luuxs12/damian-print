...
export const AuthController = {
    ...
    async verifyResetCode(request, reply, app) {
        const body = request.body;
        const ip = request.ip;
        const userAgent = request.headers["user-agent"] || "Desconocido";
        ...
        try {
            ...
            const resetToken = app.jwt.sign({ email: result.email, purpose: "reset_password" }, { expiresIn: "10m" });
            auditLogsService.createLog({
                userId: null,
                username: "Sistema (No Autenticado)",
                module: "Seguridad",
    ...
    async resetPassword(request, reply, app) {
        const body = request.body;
        const ip = request.ip;
        const userAgent = request.headers["user-agent"] || "Desconocido";
        ...
        try {
            const decoded = app.jwt.verify(body.resetToken);
