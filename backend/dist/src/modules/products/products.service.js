...
const getCategoryPrefix = (name) => {
    ...
    if (normalized === "GRAN FORMATO")
        return "GF";
    if (normalized.startsWith("GRABADO"))
        return "GRB";
...
export const updateProduct = async (id, data) => {
    ...
    if (data.manageInventory !== undefined)
        patch.manageInventory = data.manageInventory;
    if (data.countAsPrint !== undefined)
        patch.countAsPrint = data.countAsPrint;
    if (data.sendToProduction !== undefined)
        patch.sendToProduction = data.sendToProduction;
    if (data.branchName !== undefined)
        patch.branchName = data.branchName || null;
    // Advanced Pricing
    if (data.pricePublic !== undefined)
        patch.pricePublic = data.pricePublic != null ? Number(data.pricePublic) : 0.0;
    if (data.priceReseller !== undefined)
        patch.priceReseller = data.priceReseller != null ? Number(data.priceReseller) : 0.0;
    if (data.priceScales !== undefined)
        patch.priceScales = data.priceScales || null;
    if (data.specialPrices !== undefined)
        patch.specialPrices = data.specialPrices || null;
    // Costs & Materials
    if (data.laborCost !== undefined)
        patch.laborCost = data.laborCost != null ? Number(data.laborCost) : 0.0;
    if (data.overheadCost !== undefined)
        patch.overheadCost = data.overheadCost != null ? Number(data.overheadCost) : 0.0;
    if (data.materials !== undefined)
        patch.materials = data.materials || null;
    return db.transaction(async (tx) => {
...
export const calculateProductPrice = (product, qty, clientId, isReseller) => {
