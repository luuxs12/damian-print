...
export const updatePresentation = async (id, data) => {
    ...
    if (data.size !== undefined)
        patch.size = sanitize(data.size) || null;
    if (data.material !== undefined)
        patch.material = sanitize(data.material) || null;
    if (data.finish !== undefined)
        patch.finish = sanitize(data.finish) || null;
    if (data.color !== undefined)
        patch.color = sanitize(data.color) || null;
    if (data.quantity !== undefined)
        patch.quantity = sanitize(data.quantity) || null;
    if (data.imageUrl !== undefined)
        patch.imageUrl = sanitize(data.imageUrl) || null;
    if (data.cost !== undefined)
        patch.cost = Number(data.cost) || 0;
    if (data.price !== undefined)
        patch.price = Number(data.price) || 0;
    if (data.wholesalePrice !== undefined)
        patch.wholesalePrice = Number(data.wholesalePrice) || 0;
...
export const togglePresentationStatus = async (id) => {
    const [existing] = await db.select().from(presentations).where(eq(presentations.id, id));
    if (!existing)
        throw new Error("Presentación no encontrada.");
...
export const deletePresentation = async (id) => {
    const [existing] = await db.select().from(presentations).where(eq(presentations.id, id));
    if (!existing)
        throw new Error("Presentación no encontrada o ya fue eliminada.");
