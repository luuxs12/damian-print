...
export const createRole = async (data: CreateRoleDTO) => {
  ...
  if (!name || name.length < 2) throw new Error("El nombre del rol debe tener al menos 2 caracteres.");
  if (name.length > 60)         throw new Error("El nombre del rol no puede superar los 60 caracteres.");
  ...
  if (existing) throw new Error(`Ya existe un rol con el nombre "${name}".`);
    ...
    if (data.permissions?.length) {
      ...
      if (perms.length) {
        await tx.insert(rolePermissions).values(
          perms.map((p) => ({ roleId: newRole.id, permissionId: p.id }))
        );
      }
    }
...
export const updateRole = async (id: number, data: UpdateRoleDTO) => {
  ...
  if (name !== undefined) {
    ...
    if (dup && dup.id !== id) throw new Error(`Ya existe otro rol con el nombre "${name}".`);
  }
  ...
  return db.transaction(async (tx) => {
    ...
    if (data.permissions !== undefined) {
      ...
      if (data.permissions.length) {
        ...
        if (perms.length) {
          await tx.insert(rolePermissions).values(
            perms.map((p) => ({ roleId: id, permissionId: p.id }))
          );
        }
      }
    }
...
export const deleteRole = async (id: number) => {
  const [existing] = await db.select().from(roles).where(eq(roles.id, id));
  if (!existing) throw new Error("Rol no encontrado o ya fue eliminado.");
